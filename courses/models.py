...
class Course(models.Model):
    ...
    creator = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='mentored_courses'
    )
    start_date = models.DateField(_('Дата начала'))
    end_date = models.DateField(_('Дата окончания'))
    guideline = models.ForeignKey(
        Content, on_delete=models.CASCADE, verbose_name=_('Методическое указание')
    )
    participants = models.ManyToManyField(
        User, related_name='enrolled_courses', verbose_name=_('Участники')
    )
...
class LessonStatistics(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='user_statistic'
    )
...
class Lesson(models.Model):
    ...
    duration = models.DurationField(
        _('Продолжительность'), default=timedelta(minutes=60)
    )
    lesson_statics = models.ManyToManyField(
        LessonStatistics, related_name='lesson_statics'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    assignments = models.ManyToManyField(
        Content, related_name='lessons', verbose_name=_('Материалы'), blank=True
    )
    ...
    def __str__(self):
        naive_date = timezone.make_naive(self.lesson_date)
        return f"{self.name} ({self.course.name}) - {naive_date}"


class Conference(models.Model):
    zoom_id = models.URLField(_('Zoom ID'), max_length=512, unique=True)
    lesson = models.OneToOneField(
        Lesson, on_delete=models.CASCADE, related_name='conference', verbose_name=_('Занятие')
    )
...
class Test(models.Model):
    ...
    def __str__(self):
        return f"Тест для урока «{self.lesson.name}»"
